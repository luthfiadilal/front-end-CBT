...
const StudentRanking = () => {
    ...
    const handleSelectExam = async (exam) => {
        ...
        try {
            ...
            console.log('Looking for user_uid:', currentUser?.id);

            // Check if current user is in rankings
            const userInRanking = data.rankings?.find(r => r.user_uid === currentUser?.id);
            console.log('Current user in ranking:', userInRanking);
    ...
    const handleViewResult = (attemptId, resultUserUid) => {
        const currentUser = authService.getCurrentUser();

        // Students can only view their own results
        if (currentUser.role === 'siswa' && resultUserUid !== currentUser.id) {
            alert('Anda hanya dapat melihat hasil ujian Anda sendiri.');
            return;
        }
                                        ...
                                        <table className="w-full">
                                            ...
                                            <tbody className="divide-y divide-gray-200">
                                                {rankings.map((rank, index) => {
                                                    ...
                                                    const isCurrentUser = currentUser && rank.user_uid === currentUser.id;
